...
<?php
    ...
    function FileInfo(string $path): array {
        ...
        $descriptionPathname = 'art/20/description/'.$pathInfo['filename'].'.html';
        ...
        return [
            ...
            // art/20/fin/filename.jpg
            'imgSrc' => 'art/20/fin/' . $pathInfo['basename'],
            // filename.jpg
            'imgName' => $pathInfo['basename'],
            // art/20/fin/thumbsfilename.jpg
            'thumbSrc' => 'art/20/fin/thumbs/' . $pathInfo['basename'],
            // contents of filename.html
            'description' => $description,
            // art/20/description/filename.html
        ...
        <main>
          ...
          <div id="pagecontent" tabindex="0">
            ...
            <div id="archnav"></div>
              ...
              <h1><img src="img/2020.png" class="subtitle1" alt="2020"></h1>
              ...
              My partner and I started playing Final Fantasy XIV with some friends this year, so that became my main inspiration for a while. You'll see a lot of art featuring my miq'ote Skeef Deezer. There's some Guild Wars 2 as well but my interest in that was quickly taken over by FFXIV.  
                ...
                <div class="fullview" id="fvf<?= $t ?>">
                    <a href="#<?= $imgname?>"><img src="art/20/fin/<?= $imgname?>" alt="<?php if (is_string($text) && (preg_match('#^alt: (.*)#m', $text, $match))) {echo "$match[1]";}?>"></a>
                </div>
                <?php } ?>
            </div>
            ...
            <div class="gallery">
                ...
                <?php 
                $thumbs = glob("art/20/rough/thumbs/*.*");
                // gallery layout
                $thumbsCount = count($thumbs);
                foreach ($thumbs as $i => $t) { 
                // art/20/rough/thumbs/filename.jpg
                $info = Fileinfo($t);
                // contents of filename.html
                $text = $info['description'];
                // filename
                ...
                <a href="#<?= $imgname ?>"><img src="<?= $t ?>" class="galthumb" alt="<?php if (is_string($text) && (preg_match('#^alt: (.*)#m', $text, $match))) {echo "$match[1]";}?>"></a>
                <div class="expand" id="<?= $imgname?>">
                    <a href="#_"><img src="art/20/rough/<?= $imgname?>"></a>
                    <div class="meta">
                        <div class="desc">
                            <a href="#<?= "$previmg"; ?>">Previous</a> | <a href="#<?= "$nextimg"; ?>">Next</a><br>
                ...
                <div class="fullview" id="fvr<?= $t ?>">
                    <a href="#<?= $imgname?>"><img src="art/20/rough/<?= $imgname?>" alt="<?php if (is_string($text) && (preg_match('#^alt: (.*)#m', $text, $match))) {echo "$match[1]";}?>"></a>
                </div>
                <?php } ?>
            </div>
