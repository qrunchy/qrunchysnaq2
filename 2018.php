...
<?php
    ...
    function FileInfo(string $path): array {
        ...
        $descriptionPathname = 'art/18/description/'.$pathInfo['filename'].'.html';
        ...
        return [
            ...
            // art/18/fin/filename.jpg
            'imgSrc' => 'art/18/fin/' . $pathInfo['basename'],
            // filename.jpg
            'imgName' => $pathInfo['basename'],
            // art/18/fin/thumbsfilename.jpg
            'thumbSrc' => 'art/18/fin/thumbs/' . $pathInfo['basename'],
            // contents of filename.html
            'description' => $description,
            // art/18/description/filename.html
        ...
        <main>
          ...
          <div id="pagecontent" tabindex="0">
            ...
            <div id="archnav"></div>
              ...
              <h1><img src="img/2018.png" class="subtitle1" alt="2018"></h1>
              Got back in the full swing of college and had a little less time to draw. Started playing Flight Rising once again this year, while the Guild Wars 2 obsession continued. Got really absorbed into trying to design Ruby, my fourth fursona, who's been the hardest for me to settle on even a general design for. 
                ...
                <div class="fullview" id="fvf<?= $t ?>">
                    <a href="#<?= $imgname?>"><img src="art/18/fin/<?= $imgname?>" alt="<?php if (is_string($text) && (preg_match('#^alt: (.*)#m', $text, $match))) {echo "$match[1]";}?>"></a>
                </div>
                <?php } ?>
            </div>
            ...
            <div class="gallery">
                ...
                <?php 
                $thumbs = glob("art/18/rough/thumbs/*.*");
                // gallery layout
                $thumbsCount = count($thumbs);
                foreach ($thumbs as $i => $t) { 
                // art/18/rough/thumbs/filename.jpg
                $info = Fileinfo($t);
                // contents of filename.html
                $text = $info['description'];
                // filename
                ...
                <a href="#<?= $imgname ?>"><img src="<?= $t ?>" class="galthumb" alt="<?php if (is_string($text) && (preg_match('#^alt: (.*)#m', $text, $match))) {echo "$match[1]";}?>"></a>
                <div class="expand" id="<?= $imgname?>">
                    <a href="#_"><img src="art/18/rough/<?= $imgname?>"></a>
                    <div class="meta">
                        <div class="desc">
                            <a href="#<?= "$previmg"; ?>">Previous</a> | <a href="#<?= "$nextimg"; ?>">Next</a><br>
                ...
                <div class="fullview" id="fvr<?= $t ?>">
                    <a href="#<?= $imgname?>"><img src="art/18/rough/<?= $imgname?>" alt="<?php if (is_string($text) && (preg_match('#^alt: (.*)#m', $text, $match))) {echo "$match[1]";}?>"></a>
                </div>
                <?php } ?>
            </div>
